...
Sphere::Sphere(float ratio, int slices, int stacks) : ratio(ratio), slices(slices), stacks(stacks), VAO(0), VBO(0), EBO(0)
{
}
...
void Sphere::init()
{
	...
	for (int i = 0; i <= stacks; ++i)
	{
		...
		for (int j = 0; j <= slices; ++j)
		{
			...
			vertexC[i * (slices + 1) + j].position = ratio * glm::vec3(X, Y, Z);
