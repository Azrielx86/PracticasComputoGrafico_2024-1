...
int main()
{
	...
	// Variables para la moneda
	bool animaInsMoneda = false;
	float posMoneda = -4.33306;
	float movMonedaOffset = 0.05f;

	// Variables para la canica
	bool animaCanica = false;
	float posCanica = 1.20125;
	float movCanicaOffset = 0.05f;

	// Variable para ambas animaciones
	bool canReset = true;
	...
	while (!mainWindow.getShouldClose())
	{
...
#ifndef NOT_COIN_ANIMATION
		if (mainWindow.getStartCoinAnimation())
		{
			canReset = false;
			animaInsMoneda = true;
		}
		...
		{
			if (posMoneda < 0)
				posMoneda += movMonedaOffset * deltaTime;
			else
			{
				animaInsMoneda = false;
				animaCanica = true;
			}
		}
		...
		if (canReset && mainWindow.getResetAnimation())
		{
			posMoneda = -4.33306;
			posCanica = 1.20125;
		}
